...
test("it updates as the array is replaced", function() {
  ...
  run(function() {
    set(obj, 'array', A([20,21,22,23,24]));
  });
...
test("properties can be filtered by truthiness", function() {
  ...
  run(function() {
    set(obj, 'array', A([{name: "six", a:12, b:true}]));
  });
...
test("properties values can be replaced", function() {
  ...
  run(function() {
    set(obj, 'array', A([{name: 'item1', a:1, b:true}]));
  });
  ...
  module('computed.' + alias, {
    setup: function() {
      run(function() {
        obj = EmberObject.createWithMixins({
          array: A([1,2,3,4,5,6]),
          array2: A([4,5,6,7,8,9,4,5,6,7,8,9]),
          array3: A([1,8,10]),
...
test("updating sort properties updates the sorted array", function() {
  ...
  run(function() {
    set(obj, 'itemSorting', A(['fname:desc']));
  });
...
test("updating new sort properties in place updates the sorted array", function() {
  ...
  run(function() {
    set(obj, 'itemSorting', A(['age:desc', 'fname:asc']));
  });
...
test("property paths in sort properties update the sorted array", function () {
  ...
  run(function () {
    ...
    obj = EmberObject.createWithMixins({
      people: A([jaime, cersei, sansa]),
      sortProps: A(['relatedObj.status']),
      sortedPeople: computedSort('people', 'sortProps')
    });
  });
  ...
  run(function () {
    obj.set('sortProps', A(['relatedObj.firstName']));
  });
...
module('Ember.arrayComputed - chains', {
  setup: function() {
    obj = EmberObject.createWithMixins({
      todos: A([todo('E', 4), todo('D', 3), todo('C', 2), todo('B', 1), todo('A', 0)]),
