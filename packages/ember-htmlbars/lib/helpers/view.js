...
export var ViewHelper = EmberObject.create({
  ...
  helper: function(thisContext, newView, options, env) {
    var data = env.data;
  ...
  instanceHelper: function(thisContext, newView, options, env) {
    var data = env.data;
    var fn   = options.render;
...
export function viewHelper(params, options, env) {
  ...
  return ViewHelper.helper(null, viewClass, options, env); // TODO: remove first arg
}
